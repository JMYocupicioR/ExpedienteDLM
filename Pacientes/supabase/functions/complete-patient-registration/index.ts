...
const upsertPathologicalHistory = async (
  ...
  if (existing?.id) {
    return supabase.from('pathological_histories').update(payload).eq('id', existing.id);
  }

  return supabase.from('pathological_histories').insert({
    ...payload,
...
const upsertNonPathologicalHistory = async (
  ...
  if (existing?.id) {
    return supabase.from('non_pathological_histories').update(payload).eq('id', existing.id);
  }

  return supabase.from('non_pathological_histories').insert({
    ...payload,
    created_at: new Date().toISOString(),
  });
};
...
serve(async (req) => {
  ...
  try {
    ...
    // Usuario autenticado opcional para soportar idempotencia y vínculo patient_user_id
    ...
    if (tokenRow.assigned_patient_id) {
      ...
      // Reusar paciente existente del usuario autenticado para evitar duplicados.
      const { data: existingPatient } = await supabase
        .from('patients')
        .select('id, clinic_id, primary_doctor_id')
        .eq('patient_user_id', requestingUserId)
        .maybeSingle();
      ...
      if (existingPatient) {
        const { error: updateExistingErr } = await supabase
          ...
          .update({
            ...
            phone: personal.phone ?? null,
            address: personal.address ?? null,
            clinic_id: existingPatient.clinic_id ?? tokenRow.clinic_id ?? null,
            primary_doctor_id: existingPatient.primary_doctor_id ?? tokenRow.doctor_id ?? null,
            updated_at: new Date().toISOString(),
          })
          .eq('id', existingPatient.id);

        if (updateExistingErr) {
          return new Response(JSON.stringify({ error: 'No se pudo actualizar paciente existente' }), { status: 400, headers: buildCorsHeaders(req) });
        }
    ...
    // Si no había asignado y tampoco encontrado por usuario, crear nuevo paciente.
    if (!patientId) {
      const { data: patient, error: patientErr } = await supabase
        .from('patients')
        .insert({
