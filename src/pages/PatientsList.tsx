...
export default function PatientsList() {
  ...
  useEffect(() => {
    if (patientsQuery.data) {
      setPatients(patientsQuery.data as any);
    }
  }, [patientsQuery.data]);
  ...
  useEffect(() => {
    setLoading(patientsQuery.isLoading);
    setError(patientsQuery.isError ? (patientsQuery.error as any)?.message || 'Error' : null);

    if (patientsQuery.data) {
      const data = patientsQuery.data as any[];
      ...
      setQuickStats({
        ...
        recentConsultations: processedPatients.filter(
          p =>
            p.last_consultation &&
            new Date(p.last_consultation) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
        ).length,
        upcomingAppointments: processedPatients.filter(
          p => p.next_appointment && new Date(p.next_appointment) > new Date()
        ).length,
        urgentCases: processedPatients.filter(p => p.urgent_flags && p.urgent_flags.length > 0)
          .length,
      });
    }
  }, [patientsQuery.isLoading, patientsQuery.isError, patientsQuery.error, patientsQuery.data]);
  ...
  const getRecentActivity = (patient: any): string => {
    if (patient.last_consultation) {
      const daysSince = Math.floor(
        (new Date().getTime() - new Date(patient.last_consultation).getTime()) / (1000 * 3600 * 24)
      );
  ...
  const handleNewPatientCreated = (newPatient: Patient) => {
    // La lista se actualizará por invalidación; navegamos directamente
        ...
        <div className='flex items-center space-x-3'>
          ...
          <div>
            ...
            <p className='text-gray-400 text-sm'>
              {calculateAge(patient.birth_date)} años • {patient.gender}
            </p>
          </div>
        </div>
