...
export const ClinicProvider: React.FC<ClinicProviderProps> = ({ children }) => {
  ...
  const refreshUserClinics = async () => {
    try {
      ...
      console.log('✅ Refresh: clinic memberships loaded:', memberships?.length || 0, 'memberships');
  ...
  // Create a new clinic
  const createClinic = async (clinicData: { name: string; address?: string }): Promise<Clinic | null> => {
    try {
