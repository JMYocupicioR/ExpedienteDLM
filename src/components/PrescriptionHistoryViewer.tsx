...
import { useEnhancedPrescriptions } from '@/features/prescriptions/hooks/useEnhancedPrescriptions';
...
export default function PrescriptionHistoryViewer({
  ...
  const { prescriptions, getPrescriptionHistory, loading, error } = useEnhancedPrescriptions();

  const [selectedPrescription, setSelectedPrescription] = useState<EnhancedPrescription | null>(
    null
  );
  ...
  return (
    <div className='space-y-6'>
                ...
                >
                  <div className='flex items-center justify-between mb-2'>
                    <div className='flex items-center space-x-2'>
                      ...
                      <span className='text-white font-medium'>
                        {(prescription as any).patient?.first_name}{' '}
                        {(prescription as any).patient?.last_name}
                      </span>
                    </div>
                  ...
                  <div className='space-y-1'>
                    ...
                    <p className='text-sm text-gray-400'>
                      ...
                      {getMedicationsList(prescription.medications as unknown[])}
                    </p>
                  </div>
                </div>
              ))}
      ...
      {selectedPrescription && (
        <div className='space-y-4'>
          <div className='flex items-center justify-between'>
            <h3 className='text-lg font-semibold text-white'>Historial Detallado</h3>
            {showAllPrescriptions && (
              <button
              ...
              <div>
                ...
                <div className='space-y-1 text-sm'>
                  <p className='text-gray-300'>
                    <strong>Paciente:</strong> {(selectedPrescription as any).patient?.first_name}{' '}
                    {(selectedPrescription as any).patient?.last_name}
                  </p>
                  <p className='text-gray-300'>
                    ...
                    <div className='flex-1 min-w-0'>
                      ...
                      {(entry as any).performed_by_profile && (
                        <p className='text-xs text-gray-500 mt-1 flex items-center'>
                          ...
                          {(entry as any).performed_by_profile.first_name}{' '}
                          {(entry as any).performed_by_profile.last_name}
                        </p>
                      )}
                    </div>
                  </div>
