...
-- ============================================================================
-- PART 4: Create physical exam sections table for standardization
-- ============================================================================

-- Create table for standardized physical exam sections
...
-- Insert standard physical exam sections
...
-- Create physical_exam_files table if it doesn't exist
...
-- RLS policies for physical_exam_sections (read-only for all authenticated users)
...
CREATE POLICY "Medical staff can view exam files"
  ON physical_exam_files FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid()
...
CREATE POLICY "Medical staff can manage exam files"
  ON physical_exam_files FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid()
