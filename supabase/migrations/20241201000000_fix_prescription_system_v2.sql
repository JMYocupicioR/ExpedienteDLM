-- =====================================================
-- MIGRACIÓN: Corrección del Sistema de Recetas V2
-- Fecha: 2024-12-01
...
-- Función auxiliar para obtener el doctor_id del usuario actual
...
CREATE TABLE IF NOT EXISTS public.prescription_templates (
  ...
  UNIQUE(doctor_id) -- Un médico, una plantilla de formato
);
...
TO authenticated
USING (
  doctor_id IN (SELECT id FROM profiles WHERE id = auth.uid())
);
...
USING (
  EXISTS (
    ...
    AND (
      c.doctor_id IN (SELECT id FROM profiles WHERE id = auth.uid())
      OR is_admin()
    )
  )
);
...
WITH CHECK (
  EXISTS (
    SELECT 1 FROM consultations c
    WHERE c.id = consultation_id 
    AND c.doctor_id IN (SELECT id FROM profiles WHERE id = auth.uid())
  )
);
...
USING (
  EXISTS (
    ...
    AND (
      p.doctor_id IN (SELECT id FROM profiles WHERE id = auth.uid())
      OR is_admin()
    )
  )
);
...
WITH CHECK (
  EXISTS (
    SELECT 1 FROM prescriptions p
    WHERE p.id = prescription_id 
    AND p.doctor_id IN (SELECT id FROM profiles WHERE id = auth.uid())
  )
);
