/*
  ...
  1. New Tables
    - prescription_templates: Stores reusable prescription templates
      - id (uuid, primary key)
...
DROP TRIGGER IF EXISTS set_timestamp_prescription_templates ON prescription_templates;
CREATE TRIGGER set_timestamp_prescription_templates
  BEFORE UPDATE ON prescription_templates
  FOR EACH ROW
  EXECUTE FUNCTION handle_updated_at();

DROP TRIGGER IF EXISTS soft_delete_prescription_templates ON prescription_templates;
...
DROP TRIGGER IF EXISTS audit_prescription_templates ON prescription_templates;
